"""Vanilla replay buffer. Selects transitions uniformly at random.
"""
...
class UniformReplay(InterpolatedReplayBuffer):
    """Implements a uniform replay buffer. Samples are drawn uniformly, and are
    replaced in a FIFO fashion.
    """
    ...
    def interpolate_samples(self):
        ...
        # Extract transition from stored datasets
        S_t = self.X[self.sample_indices, :self.d_s]  # States/obs at t
        A_t = self.X[self.sample_indices, self.d_s:]  # Actions at t
        R_t = np.squeeze(self.Y[self.sample_indices, :self.d_r])  # Rewards at t
        S_tp1 = self.Y[self.sample_indices, self.d_r:]  # States/obs at t+1
