...
DEFAULT_CONFIG = with_common_config({
    ...
    "target_entropy": 'auto',  # NOTE: Changed from None to 'auto'
    ...
    ############################################################################
    # NOTE: Changed from 3e-4 to 1.5e-4 since taking many learning steps
    # === Optimization ===
    "optimization": {
    ...
    "target_network_update_freq": TARGET_UPDATE_FREQ["SAC"],
...
def get_policy_class_likelihood(config: TrainerConfigDict) -> Optional[Type[Policy]]:
    ...
    if config["framework"] == "torch":
        from agents.likelihood_weighted.sac.likelihood_weighted_sac_policy \
            import SACTorchPolicyLikelihoodWeighting
        return SACTorchPolicyLikelihoodWeighting
...
############################## BIER ############################################
# Likelihood-weighted agents
SACTrainerLikelihoodWeighting = GenericOffPolicyTrainer.with_updates(
    name="sac",
    default_config=DEFAULT_CONFIG,
    validate_config=validate_config,
    default_policy=SACTFPolicy,
    get_policy_class=get_policy_class_likelihood,
)

# Non-likelihood-weighted agents
