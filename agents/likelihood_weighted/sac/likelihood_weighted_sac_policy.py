...
def actor_critic_loss_likelihood_weighted(
    ...
    ############################## BIER ########################################
    # Log the magnitudes of the weights
    if parameters.GLOBAL_STEP_COUNT % parameters.LOG_INTERVAL == 0:

        # Compute the weight norms
        weight_norms = [
            float(torch.mean(torch.abs(variable)).detach().cpu().numpy())
            for variable in model.policy_variables()]
        ...
        for i, weight_norm in enumerate(weight_norms):
            if parameters.TB_WRITER is not None:  # Initialized as None
                parameters.TB_WRITER.add_scalar(
                    "SAC Metrics/Weight Norm {}".format(i), weight_norm,
    ...
    critic_loss = [
        torch.mean(
            likelihood_weights * (train_batch[PRIO_WEIGHTS] * huber_loss(base_td_error)))
    ]
    if policy.config["twin_q"]:
        critic_loss.append(
            torch.mean(
                likelihood_weights * train_batch[PRIO_WEIGHTS] * huber_loss(twin_td_error)))
    ############################## BIER ########################################
    ...
    if model.discrete:
        ...
        ############################## BIER ####################################
        # UPDATE ALPHA LOSS - DISCRETE
        alpha_loss = torch.mean(
            likelihood_weights * torch.sum(weighted_log_alpha_loss, dim=-1))
        # Actor loss.
        actor_loss = torch.mean(likelihood_weights * (
            torch.sum(
                torch.mul(
    ...
    ############################## BIER ########################################
    # UPDATE ALPHA LOSS - CONTINUOUS
    else:
        alpha_loss = -torch.mean(likelihood_weights * (model.log_alpha *
        ...
        actor_loss = torch.mean(
            likelihood_weights * (alpha.detach() * log_pis_t - q_t_det_policy))
    ############################## BIER ########################################
    ############################## BIER ########################################
    # Add l2-regularization to actor to stabilize.
    l2_reg = parameters.SAC_L2_REG[os.environ["RL_ENV"]]
    if l2_reg is not None:
        for var in policy.model.policy_variables():
            if len(var.shape) > 1:  # Don't need to regularize bias
                regularization_term = l2_loss(var)
                actor_loss += (l2_reg * regularization_term)
    ############################## BIER ########################################
    ...
    if parameters.GLOBAL_STEP_COUNT % (parameters.LOG_INTERVAL * parameters.REPLAY_RATIO) == 0:
        ...
        vals = [train_batch[PRIO_WEIGHTS], likelihood_weights, actor_loss,
                td_error, q_t, model.log_alpha, model.target_entropy,
                alpha, alpha_loss, policy_t, policy_tp1, log_pis_t, log_pis_tp1,
                q_t_det_policy, q_tp1, q_t_selected, q_tp1_best,
                q_tp1_best_masked, q_t_selected_target]
        names = ["Priority Weights", "Likelihood Weights", "Actor Loss",
                 "TD Error", "Q Values t", "Log Alpha", "Target Entropy", "Alpha",
                 "Alpha Loss", "Policy t", "Policy t+1", "Log P t", "Log P t+1",
                 "Q Values t, Deterministic", "Q Values t+1",
                 "Q Values t Selected", "Q Values t+1 Best", "Q Values t+1 Mask",
                 "Q Values t Selected Target"]
...
def actor_critic_loss(
    ...
    if parameters.CHECK_FOR_NANS:
        ...
        nans_actions = torch.any(
            torch.isnan(train_batch[SampleBatch.ACTIONS]), axis=-1)
        # NaNs rewards
        nans_rewards = torch.isnan(train_batch[SampleBatch.REWARDS])  # 1D
        # NaNs next obs
        nans_next_obs = torch.any(
            torch.isnan(train_batch[SampleBatch.NEXT_OBS]), axis=-1)

        # Group together NaNs, labels, and keys from transitions
        nan_idxes = [nans_current_obs, nans_actions,
                     nans_rewards, nans_next_obs]
        nan_labels = ["CURRENT OBS", "ACTIONS", "REWARDS", "NEXT OBS"]
        keys = [SampleBatch.CUR_OBS, SampleBatch.ACTIONS,
                SampleBatch.REWARDS, SampleBatch.NEXT_OBS]
        ...
        for nan_idx, nan_label, key in zip(nan_idxes, nan_labels, keys):
            ...
            if torch.any(nan_idx):
                print("NaNs detected in {}: \n{}".format(
                    nan_label, train_batch[key][nan_idx]))
                print("Number of {} NaNs: {}".format(nan_label,
                                                     torch.sum(nan_idx)))
                print("Setting {} NaNs to 0".format(nan_label))
    ...
    if model.discrete:
        ...
        actor_loss = torch.mean(torch.sum(torch.mul(
                    ...
                    # (compare with q_t_det_policy for continuous case).
                    policy_t,
                    alpha.detach() * log_pis_t - q_t.detach()), dim=-1))
        ############################## BIER ####################################
    ...
    if parameters.GLOBAL_STEP_COUNT % (parameters.LOG_INTERVAL * parameters.REPLAY_RATIO) == 0:
        ...
        # Get quantities and names to be logged
        vals = [train_batch[PRIO_WEIGHTS], actor_loss,
                td_error, q_t, model.log_alpha, model.target_entropy,
                alpha, alpha_loss, policy_t, policy_tp1, log_pis_t, log_pis_tp1,
                q_t_det_policy, q_tp1,
                q_t_selected, q_tp1_best, q_tp1_best_masked,
                q_t_selected_target]
        names = ["Priority Weights", "Actor Loss", "TD Error", "Q Values t",
                 "Log Alpha", "Target Entropy", "Alpha",
                 "Alpha Loss", "Policy t", "Policy t+1", "Log P t", "Log P t+1",
                 "Q Values t, Deterministic",
                 "Q Values t+1", "Q Values t Selected", "Q Values t+1 Best",
                 "Q Values t+1 Mask", "Q Values t Selected Target"]
        # Add critic loss
        if isinstance(critic_loss, list):
            vals.extend(critic_loss)
            names.extend(
...
############################## BIER ############################################
# Build a child class of `TorchPolicy`, given custom functions defined above.
# Likelihood-weighted policy
SACTorchPolicyLikelihoodWeighting = build_torch_policy(
    name="SACTorchPolicy",
    loss_fn=actor_critic_loss_likelihood_weighted,
...
# Non likelihood-weighted policy
