...
DEFAULT_CONFIG = with_common_config({
    ...
    "no_done_at_end": parameters.NO_DONE_AT_END[os.environ["RL_ENV"]],
    # Update the target by \tau * policy + (1-\tau) * target_policy.
    "tau": 5e-3,
    # Initial value to use for the entropy weight alpha.
    "initial_alpha": 0.2,  # TODO(rms): Changed from 1.0 --> 0.2
    ...
    "grad_clip": parameters.GRAD_CLIP[os.environ["RL_ENV"]],
...
def get_policy_class(config: TrainerConfigDict) -> Optional[Type[Policy]]:
    ...
    if config["framework"] == "torch":
        from agents.standard.sac.sac_torch_policy import SACTorchPolicy
