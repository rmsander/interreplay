"""Limited-Memory BFGS optimization routine adopted from:
...
def polyinterp(points, x_min_bound=None, x_max_bound=None, plot=False):
    ...
    if no_points == 2 and order == 2 and plot is False:
        ...
        if points[0, 0] == 0:
            x_sol = -points[0, 2] * points[1, 0] ** 2 / (2 * (
                        points[1, 1] - points[0, 1] - points[0, 2] * points[
                    1, 0]))
        else:
    ...
    # solve linear system
        ...
        if A.shape[0] != A.shape[1] or np.linalg.matrix_rank(A) != A.shape[0]:
            ...
            for crit_pt in crit_pts:
                if np.isreal(
                        crit_pt) and crit_pt >= x_min_bound and crit_pt <= x_max_bound:
...
class LBFGS(Optimizer):
    ...
    def __init__(self, params, lr=1., history_size=10, line_search='Wolfe',
        ...
        defaults = dict(lr=lr, history_size=history_size,
                        line_search=line_search, dtype=dtype, debug=debug)
