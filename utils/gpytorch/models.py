"""Classes and utility functions for GPyTorch GPR models. These models are
defined for the following applications to reinforcement learning:

1. Batched GPR models (BatchedGP): This model class is recommended for
    practicality in reinforcement learning domains. This model class
    support batching across both multiple data points and dimensions
    simultaneously. It is important to note that each outcome dimension is
    modeled independently. Can be used in the training function
    gpr_train_batch in gpytorch_utils.py.

2. Batched Composite Kernel GPR models (CompositeBatchedGP): Identical to the
    BatchedGP class, except implements the kernel as a product over states and
    actions, rather than as a single concatenated state-action vector.

3. Single-dimensional GPR (GPModel): Performs standard GPR using a single target
    (y) dimension without batching. Can be used in a "batched" setting using the
    training function train_gpytorch_modellist in gpytorch_utils.py.

4. Multi-dimensional GPR (MultitaskGPModel): Performs GPR using possibly
    multiple target (y) dimensions without batching. Can be used in a "batched"
    setting using the training function train_gpytorch_modellist in
    gpytorch_utils.py.


This module also contains utility functions for obtain mean/kernel functions
and priors for the mean and covariance modules of these classes.

A few notes for training GPR models:

1. Default priors can be used (i.e. no priors need to be specified) if:
    (i) Targets are standardized to N(0, 1), and
    (ii) Features min-max normalized to [0, 1].

2. Automatic Relevance Determination (ARD) is strongly encouraged for RL
    environments, because of the high variation in scale between each input
    dimension (state/action spaces) and the relevance that these dimensions
    play in determining predicted outputs.
"""
# External Python packages
import gpytorch
from gpytorch.models import ExactGP
from gpytorch.means import ConstantMean, ZeroMean, MultitaskMean, LinearMean
from gpytorch.kernels import RQKernel, RBFKernelGrad, ScaleKernel, MultitaskKernel
...
class BatchedGP(ExactGP):
    """Independent Batched GPR models that support hardware-accelerated
    operations. Kernel defined over concatenated state-action space.

    Class for creating batched Gaussian Process Regression models.
    Ideal candidate if using GPU-based acceleration such as CUDA for training.
    Can be used in the training function gpr_train_batch in
    gpytorch_utils.py.
    ...
    Parameters:
        ...
        kernel (str): Type of kernel to use for optimization. Defaults to "
            "Radial Basis Function kernel (RBF) / Squared Exponential ('rbf').
            Other options include Matern ('matern'), and RQ (Rational Quadratic)
            ('rq').
    ...
    def __init__(self, train_x, train_y, likelihood, shape, output_device,
                 use_ard=False, use_priors=False, kernel='rbf',
                 mean_type='zero', matern_nu=2.5, heuristic_lengthscales=None,
                 lengthscale_prior_std=0.1, lam=0.0):
        ...
        self.base_kernel = K(batch_shape=self.shape,
                             ...
                             lengthscale_prior=lengthscale_prior, **kwargs)
    ...
    def forward(self, x):
        ...
        Returns:
            mv_normal (gpytorch.distributions.MultivariateNormal): A Multivariate
                Normal distribution with parameters for mean and covariance
                computed at x.
        """
        mean_x = self.mean_module(x)  # Compute the mean at x
        covar_x = self.covar_module(x)  # Compute the covariance at x

        # Perform covariance regularization
        return MultivariateNormal(mean_x, covar_x)


class CompositeBatchedGP(ExactGP):
    """Independent Batched GPR models that support hardware-accelerated
    operations. Kernel defined over product of kernels, one for state space,
    and one for action space.

    Class for creating batched Gaussian Process Regression models. Ideal GPR
    model if using GPU-based acceleration such as CUDA for training. This model
    produces a composite kernel that multiplies actions times states, i.e. we
    have a different kernel for both the actions and states. In turn, the
    composite kernel is then multiplied by a Scale kernel.
    ...
    def __init__(self, train_x, train_y, likelihood, shape, output_device,
                 ...
                 kernel='rbf', mean_type='zero', matern_nu=2.5,
        ...
        if ds is None:
            raise Exception(
                "No dimension for state specified. Please specify ds.")
        self.ds = ds

        # Set active dimensions for each sub-kernel
        ...
        # Get mean function and kernel
        M, K_state, K_action, kwargs = get_mean_and_kernel(
            kernel, mean_type, shape, input_size,
            is_composite=True, matern_nu=matern_nu)
